...
#define TLINE(y)                                                               \
  ((y) < term.scr                                                              \
       ? term.hist[(term.histi + (y)-term.scr + 1 + HISTSIZE) % HISTSIZE]      \
...
#define UPDATEWRAPNEXT(alt, col)                                               \
  do {                                                                         \
    if ((term.c.state & CURSOR_WRAPNEXT) &&                                    \
        term.c.x + term.wrapcwidth[alt] < col) {                               \
...
int tiswrapped(Line line) {
  int len = tlinelen(line);

  return len > 0 && (line[len - 1].mode & ATTR_WRAP);
}

char *tgetglyphs(char *buf, const Glyph *gp, const Glyph *lgp) {
  while (gp <= lgp)
...
size_t tgetline(char *buf, const Glyph *fgp) {
  char *ptr;
...
int regionselected(int x1, int y1, int x2, int y2) {
...
int selected(int x, int y) { return regionselected(x, y, x, y); }
...
int tisaltscreen(void) { return IS_SET(MODE_ALTSCREEN); }
...
void tscrolldown(int top, int n) {
  int i, bot = term.bot;
  Line temp;
...
void tscrollup(int top, int bot, int n, int mode) {
  int i, j, s;
  int alt = IS_SET(MODE_ALTSCREEN);
...
void selscroll(int top, int bot, int n) {
...
void tclearregion(int x1, int y1, int x2, int y2, int usecurattr) {
  int x, y;

  /* regionselected() takes relative coordinates */
  if (regionselected(x1 + term.scr, y1 + term.scr, x2 + term.scr,
                     y2 + term.scr))
    selremove();
...
void csihandle(void) {
  ...
  case 'n': /* DSR – Device Status Report (cursor position) */
    if (csiescseq.arg[0] == 6) {
      n = snprintf(buf, sizeof(buf), "\033[%i;%iR", term.c.y + 1, term.c.x + 1);
      ttywrite(buf, n, 0);
    }
    break;
